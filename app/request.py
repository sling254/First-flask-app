...
def get_movie(id):
    ...
    with urllib.request.urlopen(get_movie_details_url) as url:
        ...
        movie_detail_response = json.loads(movie_details_data)

        movie_object = None

        if movie_detail_response:
            id = movie_detail_response.get('id')
            title = movie_detail_response.get('title')
...
def search_movie(movie_name):
    search_movie_url = search_movie_url = 'https://api.themoviedb.org/3/search/movie?api_key={}&query={}'.format(api_key,movie_name)
    with urllib.request.urlopen(search_movie_url)as url:
        search_movie_data = url.read()
        search_movie_response = json.loads(search_movie_data)

        search_movie_results = None
        if search_movie_response:
